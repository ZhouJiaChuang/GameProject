...
sys.path.append("../proto-table_lua")
...
def ParseIntPair(row_value):
    ...
    lenght = int(len(list))
    if lenght >= 1:
        pair.first = int(list[0])
    if lenght >= 2:
        pair.second = int(list[1])
    return pair
def ParseIntPair2(row_value):
    INFO("int_pair = " + row_value)
    TRACE("----------------一对数字a-----------------------")
    INFO("int_pair = " + row_value)
    pair = table_common_pb2.IntPair()
...
def ParseIntPairList2(row_value):
    int_list_list = table_common_pb2.IntPairList2()
    list_list = row_value.split('&')
    for data in list_list:
        pair = int_list_list.list.add()
        list = data.split('#')
        lenght = int(len(list))
        if lenght >= 1:
           pair.first = int(list[0])
...
def ParseIntListJingHaoMeiYuan(row_value):
    ...
    for f_list in list_list:
        ...
        for v in int_list:
            internal_list.list.append(int(v))
    return int_list_list
def ParseIntListXiaHuaXianFenHao(row_value):
...
def Process(workbook_name, sheet_name, entry_name):
    ...
    for desc in getattr(module, entry_name).DESCRIPTOR.fields:
        ...
        if desc.type == desc.TYPE_INT32 or \
            ...
            elif msg_desc.name == "IntPairList2":
                field_type = table_common_pb2.IntPairList2
    ...
    for curr_row in xrange(sheet.nrows):
        if curr_row < 4:   # first line is desc
            continue
        ...
        for field_desc in proto_desc:
            ...
            elif field_desc[ENUM_FIELD_TYPE] == table_common_pb2.IntPairList2:
                if row_value != "":
                    int_pair_list = ParseIntPairList2(row_value)
                    if not int_pair_list:
                        ERROR("pass IntPairList failed: " + row_value);
                        exit(1)
                    getattr(row, field_desc[ENUM_FIELD_NAME]).CopyFrom(int_pair_list)
